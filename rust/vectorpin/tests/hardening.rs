...
fn pin_from_json_rejects_non_string_extra_value() {
    ...
    assert!(
        matches!(res, Err(AttestationError::ExtraTypeMismatch(ref s)) if s == "k"),
        "expected ExtraTypeMismatch(\"k\"), got {:?}",
        res
    );
}
...
fn pin_from_json_accepts_string_extra_value() {
    ...
    let pin = Pin::from_value(ok).expect("string-valued extra parses");
    assert_eq!(pin.header.extra.get("k").map(String::as_str), Some("v"));
}
